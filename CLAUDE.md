...
**Localization:** All UI strings go through `AppLocalizations.of(context)`. Source of truth is `lib/l10n/app_en.arb`. The `.dart` files in `lib/l10n/` are generated by `flutter gen-l10n` — `pubspec.yaml` has `generate: true`, so this also runs automatically on every `flutter build`/`flutter run` and will silently overwrite any hand edits to `app_localizations*.dart`. Edit the `.arb` files to change strings, and edit `l10n.yaml` to change codegen behavior (e.g. `use-deferred-loading: true` is set there so each locale compiles to its own lazy-loaded JS chunk on web instead of bloating `main.dart.js`).
